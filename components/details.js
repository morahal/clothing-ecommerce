...
const DetailsPage = ({ route }) => {
  ...
  useEffect(() => {
    const checkItemAndAddToBag = async () => {
      if (selectedOptions.size && selectedOptions.colour) {
        try {
          const url = `${BASE_URL}/specificItem/?section=${encodeURIComponent(
            item.section
          )}&category=${encodeURIComponent(
            item.category
          )}&name=${encodeURIComponent(item.name)}&size=${encodeURIComponent(
            selectedOptions.size
          )}&color=${encodeURIComponent(selectedOptions.colour)}`;
          ...
          if (response.ok) {
            ...
            bagDispatch({
              ...
              payload: {
                ...data,
                s_ize: selectedOptions.size,
                colour: selectedOptions.colour,
              },
            });
            ...
            Alert.alert(
              "The size or color chosen for this item are out of stock."
            );
            setSelectedOptions({ size: null, colour: null }); // Reset selected options
          }
        } catch (error) {
